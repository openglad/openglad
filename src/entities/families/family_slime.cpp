...
{
    self->dead = 1;
    walker* newob = og::gameplay::current_game->world->add_ob(Order::Living, FAMILY_MEDIUM_SLIME);
...
{
    self->dead = 1;
    walker* newob = og::gameplay::current_game->world->add_ob(Order::Living, FAMILY_SMALL_SLIME);
    newob->team_num = self->team_num;
    newob->stats()->level = self->stats()->level;
    newob->set_difficulty(self->stats()->level);
    newob->foe = self->foe;
...
static bool small_slime_do_special(walker* self)
{
    ...
    {
        self->stats()->set_command(COMMAND_WALK, 10, static_cast<std::int32_t>(og::gameplay::current_game->world->rng_.next(3)) - 1, static_cast<std::int32_t>(og::gameplay::current_game->world->rng_.next(3)) - 1);
        return false;
    }
    return true;
}
