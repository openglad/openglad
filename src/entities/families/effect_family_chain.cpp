...
static bool chain_on_act(effect* self)
{
    ...
    {
        walker* newob = og::gameplay::current_game->world->add_ob(Order::FX, FAMILY_EXPLOSION);
        if (!newob)
        {
            self->dead = 1;
            self->death();
        ...
        if (self->owner->myguy)
            foelist = og::gameplay::current_game->world->find_foes_in_range(og::gameplay::current_game->world->oblist,
        ...
        if (temp && generic>20)
        {
            std::int32_t numfoes = static_cast<std::int32_t>(og::gameplay::current_game->world->rng_.next(static_cast<std::uint32_t>(self->owner->stats()->level))) + 1;
            for(auto* w : foelist)
            {
                if (numfoes <= 0) break;
                if (w != self->leader && w->skip_exit<1)
                {
                    newob = og::gameplay::current_game->world->add_ob(Order::FX, FAMILY_CHAIN);
                    if (!newob)
                        return true;
                    newob->owner = self->owner;
                    newob->leader = w;
