...
static void archmage_hit_response(statistics* stats, walker* foe)
{
    ...
    if (stats->hitpoints < threshold && possible_specials[1] && og::gameplay::current_game->world->rng_.next(3))
    {
    ...
    switch (self->current_special)
    {
        ...
        case 3: // summon image / elemental
            ...
            if (self->shifter_down) // true summoning
            {
                ...
                {
                    if (self->user != -1)
                        og::sim::emit_notification(og::gameplay::current_game->sim_events, "150 Int required to Summon!");
                    return false;
                }
                generic = static_cast<std::int32_t>(self->stats()->magicpoints - static_cast<float>(self->stats()->special_cost[3]));
                generic /= 2;
                self->stats()->magicpoints -= static_cast<float>(generic);
                newob = og::gameplay::current_game->world->add_ob(Order::Living, FAMILY_FIREELEMENTAL);
                if (!newob)
                    return false;
                generic = 0;
                for (i = -1; i <= 1; i++)
                    for (j = -1; j <= 1; j++)
                    {
                        ...
                        if (og::gameplay::current_game->world->query_passable(testx, testy, newob))
                        {
                            generic = 1;
            ...
            else // illusion summoning
            {
                ...
                {
                    switch (og::gameplay::current_game->world->rng_.next(3))
                    {
                ...
                {
                    switch (og::gameplay::current_game->world->rng_.next(5))
                    {
                        case 0: person = FAMILY_ELF; break;
                        case 1: person = FAMILY_SOLDIER; break;
                        case 2: person = FAMILY_ARCHER; break;
                ...
                {
                    switch (og::gameplay::current_game->world->rng_.next(7))
                    {
                        case 0: person = FAMILY_ELF; break;
                        case 1: person = FAMILY_SOLDIER; break;
                        case 2: person = FAMILY_ARCHER; break;
                ...
                {
                    switch (og::gameplay::current_game->world->rng_.next(9))
                    {
                        case 0: person = FAMILY_ELF; break;
                        case 1: person = FAMILY_SOLDIER; break;
                        case 2: person = FAMILY_ARCHER; break;
                ...
                newob = og::gameplay::current_game->world->add_ob(Order::Living, person);
                if (!newob)
                    return false;
                generic = 0;
                for (i = -1; i <= 1; i++)
        ...
        case 4: // mind control
        {
            ...
            std::int32_t controlled_targets = 0;
            {
                ...
                for (auto* ob : newlist)
                {
                        ...
                        (ob->charm_left <= 10))
                    {
                        ...
                        if (generic < 0 || (!og::gameplay::current_game->world->rng_.next(20)))
                        {
                            ob->real_team_num = ob->team_num;
                            ob->team_num = static_cast<unsigned char>(og::gameplay::current_game->world->rng_.next(8));
                            ob->charm_left = (static_cast<short>(compute_charm_duration(generic, og::gameplay::current_game->world->rng_)));
                        }
                        else
                        {
                            ob->real_team_num = ob->team_num;
            ...
            og::sim::emit_notification(og::gameplay::current_game->sim_events, tempstr);
