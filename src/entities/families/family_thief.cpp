...
static bool thief_do_special(walker* self)
{
    ...
    {
        case 1: // drop bomb
            newob = og::gameplay::current_game->world->add_ob(Order::FX, FAMILY_BOMB, 1);
            ...
            // Run away if we're AI
            {
                if (self->user == -1)
                {
                    std::int32_t tempx = static_cast<std::int32_t>(og::gameplay::current_game->world->rng_.next(3)) - 1;
                    std::int32_t tempy = static_cast<std::int32_t>(og::gameplay::current_game->world->rng_.next(3)) - 1;
                    if ((tempx == 0) && (tempy == 0))
                        tempx = 1;
            ...
            break;
        case 2: // cloak
            self->invisibility_left = static_cast<short>(self->invisibility_left + 20 + static_cast<std::int32_t>(og::gameplay::current_game->world->rng_.next(20)) * self->stats()->level);
            break;
        case 3: // taunt / charm
            if (!self->shifter_down) // normal taunt
            {
                ...
                for_each_foe_in_range(self, 80 + 4 * self->stats()->level, [self](walker* ob) {
                    if (og::gameplay::current_game->world->rng_.next(self->stats()->level) >= og::gameplay::current_game->world->rng_.next(ob->stats()->level))
                    {
                        ob->foe = self;
                        ob->leader = self;
                        if (ob->act_type != ACT_CONTROL)
                            ob->stats()->force_command(COMMAND_FOLLOW, 10 + og::gameplay::current_game->world->rng_.next(self->stats()->level), 0, 0);
                    }
                });
