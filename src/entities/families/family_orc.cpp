...
static bool orc_do_special(walker* self)
{
    ...
    switch (self->current_special)
    {
        case 1: // yell and 'freeze' foes
            ...
            {
                std::list<walker*> newlist = og::gameplay::current_game->world->find_foes_in_range(
                    og::gameplay::current_game->world->oblist,
                ...
                for (auto* ob : newlist)
                {
                    if (ob)
                    {
                        ...
                        tempy = 10
                            + static_cast<std::int32_t>(og::gameplay::current_game->world->rng_.next(static_cast<std::uint32_t>(self->stats()->level * 10)))
                            - static_cast<std::int32_t>(og::gameplay::current_game->world->rng_.next(static_cast<std::uint32_t>(tempx_clamped * 10)));
                        if (tempy < 0)
                            tempy = 0;
                ...
                og::sim::emit_sound(og::gameplay::current_game->sim_events, SOUND_ROAR);
            }
            break;
