...
bool living::act()
{
	...
	if (foe && (foe->dead || (og::gameplay::current_game->world->rng_.next(foe->invisibility_left/20) > 0) ) )
		foe = nullptr;
	if (is_friendly(foe))
		foe = nullptr;
	...
	*/
	...
	// Regenerate magic
	{
		bool frozen = og::gameplay::current_game->world->enemy_freeze || bonus_rounds;
	...
	if (flight_left > 0)
		flight_left--;
	if (!og::gameplay::current_game->world->query_grid_passable(xpos, ypos, this) && !flight_left)
	{
		flight_left++;
		stats_->hitpoints--;
	...
	if ( stats_->query_bit_flags(BIT_FORESTWALK) &&
	        (
	            og::gameplay::current_game->world->mysmoother.query_genre_x_y( xpos/GRID_SIZE, ypos/GRID_SIZE) == TYPE_TREES
	            || og::gameplay::current_game->world->mysmoother.query_genre_x_y( (xpos+sizex)/GRID_SIZE, ypos/GRID_SIZE) == TYPE_TREES
	            || og::gameplay::current_game->world->mysmoother.query_genre_x_y( (xpos+sizex)/GRID_SIZE, (ypos+sizey)/GRID_SIZE) == TYPE_TREES
	            || og::gameplay::current_game->world->mysmoother.query_genre_x_y( xpos/GRID_SIZE, (ypos+sizey)/GRID_SIZE) == TYPE_TREES
	        )
	   )
	{
	...
	switch (act_type)
	{
		...
		case ACT_RANDOM:
			{
				if (!og::gameplay::current_game->world->rng_.next(5) ) //1 in 5 to do our special
				{
					// Should we do our special? Are we full of magic?
					if (stats_->magicpoints >= stats_->special_cost[1])
					{
						current_special = static_cast<char>(og::gameplay::current_game->world->rng_.next((stats_->level+2)/3) + 1);
				...
				else if (!og::gameplay::current_game->world->rng_.next(5) ) //1 in 5 to do act_random() function
					act_random();
				else // 4 of 5 times
				{
					if (!foe)
					...
					if (foe) // && og::gameplay::current_game->world->rng_.next(2) )
					{
					...
					else if (!og::gameplay::current_game->world->rng_.next(2))
...
short living::shove(walker  *target, short x, short y)
{
	...
	if (target && !target->dead && (query_order()==Order::Living) &&  //we are alive
		...
		if (og::gameplay::current_game->world->rng_.next(3) && target->act_type != ACT_CONTROL)
		{
...
bool living::walk(float x, float y)
{
	...
	if (curdir == dir)  // if continue direction
	{
		...
		if (og::gameplay::current_game->world->query_passable(xpos+x, ypos+y,this) && og::gameplay::current_game->world->query_passable(xpos+ceilf(x), ypos+ceilf(y),this) && og::gameplay::current_game->world->query_passable(xpos+floorf(x), ypos+floorf(y),this))
		{
...
{
	shifter_down = static_cast<short>(og::gameplay::current_game->world->rng_.next(2)); // on or off, randomly ..
...
bool living::act_random()
{
	...
	// Find our foe
	if (!og::gameplay::current_game->world->rng_.next(80) || (!foe))
		foe = og::gameplay::current_game->world->find_near_foe(this);
	if (!foe)
	        ...
	        abs(ydist) < lineofsight*GRID_SIZE)
	{
		if (fire_check(xdist, ydist))
		{
			...
			stats_->set_command(COMMAND_FIRE, static_cast<short>(og::gameplay::current_game->world->rng_.next(24)), xdist, ydist);
			return 1;
		}
		else
...
bool living::do_action()
{
	...
	switch (action)
	{
		case ACTION_FOLLOW: // follow our leader, attack his targets ..
			...
			leader = og::gameplay::current_game->world->find_nearest_player(this);
			if (!leader)
