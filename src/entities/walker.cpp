...
walker  * walker::fire()
{
	...
	waver = static_cast<signed char>(og::gameplay::current_game->world->rng_.next(waver+1) - waver/2);
	...
	// Actual combat
	if (!og::gameplay::current_game->world->query_passable(weapon->xpos, weapon->ypos, weapon))
	{
		// *** Melee combat ***
	...
	else
	{
		...
		// *** Ranged combat ***
		{
			...
			og::sim::emit_sound(og::gameplay::current_game->sim_events, static_cast<std::uint32_t>(wfd ? wfd->fire_sound : SOUND_FWIP));
		}
		if (order == Order::Generator)
		{
			...
			if (gfd)
			{
				...
				weapon->stats()->level = static_cast<std::int32_t>(og::gameplay::current_game->world->rng_.next(static_cast<std::uint32_t>(stats_->level))) + 1;
...
bool walker::act()
{
	...
	switch (act_type)
	{
		...
		case ACT_RANDOM:
			{
				if (!og::gameplay::current_game->world->rng_.next(4) )
				{
					if (!og::gameplay::current_game->world->rng_.next(20))   // a 1 in 4 then 1 in 20 chance of rand walk
					{
						if (!special())
							stats_->try_command(COMMAND_WALK, og::gameplay::current_game->world->rng_.next(30),
							                   og::gameplay::current_game->world->rng_.next(3)-1, og::gameplay::current_game->world->rng_.next(3)-1);
						return 1;
					}
				...
				else    //3 of 4 times
				{
					if (!foe)
					{
						foe = og::gameplay::current_game->world->find_far_foe(this);
					}
					if (foe)
...
bool walker::query_next_to()
{
	...
	if (!og::gameplay::current_game->world->query_object_passable(newx, newy, this))
	{
		return 1;
	}
	else
...
bool walker::fire_check(short xdelta, short ydelta)
{
	...
	for (i=0; i < weapon->lineofsight; i++)
	{
		...
		if ( !og::gameplay::current_game->world->query_grid_passable(weapon->xpos, weapon->ypos, weapon) )
		{
			// we hit a wall, so fail
			weapon->dead = 1;
			return 0;
		}
		if ( !og::gameplay::current_game->world->query_object_passable(weapon->xpos, weapon->ypos, weapon) )
		{
...
walker::act_generate()
{
	if ( og::gameplay::current_game->world->living_count < MAXOBS &&
	        (og::gameplay::current_game->world->rng_.next(static_cast<std::uint32_t>(stats_->level * 3)) > og::gameplay::current_game->world->rng_.next(static_cast<std::uint32_t>(300 + (og::gameplay::current_game->world->living_count * 8))) )
	   )
	{
		lastx = static_cast<float>(1 - static_cast<std::int32_t>(og::gameplay::current_game->world->rng_.next(3)));
		lasty = static_cast<float>(1 - static_cast<std::int32_t>(og::gameplay::current_game->world->rng_.next(3)));
		if (!lastx && !lasty)
			lastx = 1;
...
walker::act_guard()
{
	...
	foe = og::gameplay::current_game->world->find_near_foe(this);
	if (foe)
	{
		curdir = static_cast<char>(facing(foe->xpos - xpos, foe->ypos-ypos));
		stats_->try_command(COMMAND_FIRE,og::gameplay::current_game->world->rng_.next(30));
		return 1;
	}
	else
		return 0;
...
walker::act_random()
{
	...
	//if (og::gameplay::current_game->world->rng_.next(sizex/GRID_SIZE)) return 0;

	// Find our foe
	if (!og::gameplay::current_game->world->rng_.next(70) || (!foe))
		foe = og::gameplay::current_game->world->find_far_foe(this);
	if (!foe)
	        ...
	        abs(ydist) < lineofsight*GRID_SIZE)
	{
		if (fire_check(xdist, ydist))
		{
			...
			stats_->set_command(COMMAND_FIRE, og::gameplay::current_game->world->rng_.next(24), xdist, ydist);
			return 1;
		}
		else
	...
	if (foe)
	{
		...
		else
		{
			while ( !newx && !newy)
			{
				newx = static_cast<short>(1 - static_cast<std::int32_t>(og::gameplay::current_game->world->rng_.next(3)));   // Walk in some random direction
				newy = static_cast<short>(1 - static_cast<std::int32_t>(og::gameplay::current_game->world->rng_.next(3)));   // other than 0,0 :)
			}
		}
	...
	for (i=-1; i < 2; i++)
		for (j=-1; j < 2; j++)
			if (i || j) // don't check our own location
				if (og::gameplay::current_game->world->query_passable(xpos+(i*sizex), ypos+(j*sizey), this) )
					count++;
...
void walker::transform_to(Order whatorder, std::int32_t whatfamily)
{
	...
	if (og::gameplay::current_game && og::gameplay::current_game->world && og::gameplay::current_game->world->myobmap)
		og::gameplay::current_game->world->myobmap->remove(this);
	...
	switch (order)
	{
		case Order::Living:
			if (   (team_num == 0 || myguy) // our team
			        && (og::gameplay::current_game->world->type & SCEN_TYPE_SAVE_ALL)
		...
		case Order::Generator:  // go up in flames :>
			for (i=0; i < 4; i++)
			{
				newob = og::gameplay::current_game->world->add_ob(Order::FX, FAMILY_EXPLOSION, 1);
				if (!newob) // failsafe
					break;
				newob->team_num = team_num;
				newob->stats()->level = stats_->level;
				newob->ani_type = ANI_EXPLODE;
				newob->setxy(xpos+og::gameplay::current_game->world->rng_.next(sizex-8)+4, ypos+4+og::gameplay::current_game->world->rng_.next(sizey-8) );
					newob->damage = static_cast<float>(stats_->level) * 2.0f;
					newob->set_frame(static_cast<short>(og::gameplay::current_game->world->rng_.next(3)));
...
void walker::generate_bloodspot()
{
	...
	bloodstain->set_frame(static_cast<short>(og::gameplay::current_game->world->rng_.next(4)));  // has no effect yet ..
