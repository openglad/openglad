...
static bool bomb_on_death(effect* self)
{
    ...
    walker* newob = og::gameplay::current_game->world->add_ob(Order::FX, FAMILY_EXPLOSION, 1);
...
static bool explosion_on_death(effect* self)
{
    ...
    auto foelist = og::gameplay::current_game->world->find_in_range(og::gameplay::current_game->world->oblist, 15+generic,
        &howmany, self);

    // Emit DamageTile event instead of calling screen directly
    og::sim::emit_event(og::gameplay::current_game->sim_events, og::sim::EventKind::DamageTile,
