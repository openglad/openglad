...
static bool cloud_on_act(effect* self)
{
    ...
    auto foelist = og::gameplay::current_game->world->find_foes_in_range(
    ...
    else
    {
        ...
        while (xd == 0 && yd == 0)
        {
            xd = static_cast<float>(static_cast<std::int32_t>(og::gameplay::current_game->world->rng_.next(3)) - 1);
            yd = static_cast<float>(static_cast<std::int32_t>(og::gameplay::current_game->world->rng_.next(3)) - 1);
        }
        self->stats()->add_command(COMMAND_WALK, static_cast<short>(og::gameplay::current_game->world->rng_.next(20)),
