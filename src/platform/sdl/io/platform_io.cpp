...
void io_init(int argc, char* argv[])
{
    ...
    if(!og::resources::mount(get_user_path(), nullptr, 1))
    {
    ...
    {
        std::string msg = std::format("Fatal: Failed to mount default campaign: {}", og::resources::last_error());
        LogError("{}\n", msg);
        throw std::runtime_error(msg);
    }
    ...
    if(!og::resources::mount(get_asset_path() + "pix/", "pix/", 1))
    {
        LogWarn("Failed to mount default pix path (may be bundled in campaign)\n");
    }
    if(!og::resources::mount(get_asset_path() + "sound/", "sound/", 1))
    {
        LogWarn("Failed to mount default sound path (may be bundled in campaign)\n");
    }
    if(!og::resources::mount(get_asset_path() + "cfg/", "cfg/", 1))
    {
...
NewFileIoError create_new_scen_file_with_error(const std::string& scenfile, const std::string& gridname)
{
    ...
    if (!og::data::create_new_scenario(scenfile, gridname, &io_error))
    {
        if (io_error == og::data::LevelFileIoError::OpenReadFailed)
            return NewFileIoError::OpenWriteFailed;
