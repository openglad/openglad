...
void show_guy(Sint32 frames, guy* myguy, Sint32 centerx, Sint32 centery) // shows the current guy ..
{
	...
	mywalker = og::runtime::current_session->myscreen_->world().entity_factory(Order::Living,
	           newfamily);
...
int get_num_foes(og::gameplay::GameWorld& level)
{
    int result = 0;
...
bool results_screen(int ending, int nextlevel, std::map<int, guy*>& before, std::map<int, walker*>& after)
{
    ...
    og::gameplay::GameWorld& level_data = og::runtime::current_session->myscreen_->world();
