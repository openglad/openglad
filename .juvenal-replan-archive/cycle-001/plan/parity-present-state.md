...
Final metrics for the parity hardening effort. Captured after Phase 11
(final sweep) at branch `wip/networking`.

## Scenario Inventory

- **Total scenarios in `kScenarios[]`:** 134
- **`CompareMode::SemanticParity`:** 132
- **`CompareMode::Invariant`:** 2 (`snapshot_dirty_bits_scen9301`,
  `smoke_empty_scen99`)
- **`CompareMode::ByteEqual`:** 0
- **Golden JSON files in `tests/parity/golden/`:** 133
...
- **Total GoogleTest cases across integration binaries:** 1873.
- **Total GoogleTest cases across unit binaries:** 530.
- **Per-scenario parity cases in `og_test_parity`:** 160 (134 scenarios +
  26 additional fixtures including the 7 depth/quality gate tests
  and dumper-determinism/coverage checks). All 160 pass.
...
- Total `FactPredicate` arrays bound to scenarios: 132 (one per
  SemanticParity scenario).
- Total predicates across all arrays: 626.
- Average predicate depth: **4.74** non-empty predicates per scenario.
...
- **Behavioral** consequence coverage: every weapon emission, effect
  emission, special ability, treasure pickup, generator, and event
  scenario carries at least one consequence predicate
  (`WalkerHpRangeAtFinalTick`, `WalkerDiedByFinal`, `WalkerPositionMoved`,
  `WeaponFamilyEmitted`, `EffectFamilyCount`, `ScoreDelta`,
  `TreasurePickup`, `TreasureFamilyRemovedFromOblist`, or a cross-family
  `WalkerFamilyCount` distinct from the wielder).
...
- The 7 depth/quality gates in `test_parity_depth_gates.cpp` enforce that
...
see `tests/parity/test_parity_depth_gates.cpp` for the enforcing
gate that fails the build if a widened range lacks a justification.
