...
void test_video_fadeblack_smoke_in_and_out()
{
    ...
    int r1 = myscreen->fadeblack(true);
    int r2 = myscreen->fadeblack(false);
...
void test_video_fadebetween_precondition_failures_are_handled()
{
    ...
    int r = myscreen->FadeBetween(old_ok.get(), new_bad_w.get(), dest.get());
...
void test_video_fadebetween_success_path_smoke()
{
    ...
    int r = myscreen->FadeBetween(old_ok.get(), new_ok.get(), dest.get());
...
void test_video_fadebetween24_smoke()
{
    ...
    myscreen->FadeBetween24(s.get(), from.data(), to.data(), 1);
}
