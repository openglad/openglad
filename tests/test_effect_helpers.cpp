...
TEST(EffectHelpers, orbit_offset_north)
{
    float xd = 999, yd = 999;
    orbit_offset(0, xd, yd);
    ASSERT_EQ(0, static_cast<int>(xd)) << "drawcycle 0 should be due north (x=0)";
    ASSERT_EQ(-24, static_cast<int>(yd)) << "drawcycle 0 should be due north (y=-24)";
}


TEST(EffectHelpers, orbit_offset_south)
{
    float xd = 999, yd = 999;
    orbit_offset(8, xd, yd);
    ASSERT_EQ(0, static_cast<int>(xd)) << "drawcycle 8 should be due south (x=0)";
    ASSERT_EQ(24, static_cast<int>(yd)) << "drawcycle 8 should be due south (y=24)";
}


TEST(EffectHelpers, orbit_offset_wraps_at_16)
{
    ...
    ASSERT_EQ(static_cast<int>(xd0), static_cast<int>(xd16)) << "drawcycle 16 should wrap to match drawcycle 0 (x)";
    ASSERT_EQ(static_cast<int>(yd0), static_cast<int>(yd16)) << "drawcycle 16 should wrap to match drawcycle 0 (y)";
}


TEST(EffectHelpers, orbit_offset_west_and_east)
{
    float xd = 0, yd = 0;
    orbit_offset(4, xd, yd);
    ASSERT_EQ(-24, static_cast<int>(xd)) << "drawcycle 4 should be due west (x=-24)";
    ASSERT_EQ(0, static_cast<int>(yd)) << "drawcycle 4 should be due west (y=0)";

    orbit_offset(12, xd, yd);
    ASSERT_EQ(24, static_cast<int>(xd)) << "drawcycle 12 should be due east (x=24)";
    ASSERT_EQ(0, static_cast<int>(yd)) << "drawcycle 12 should be due east (y=0)";
}
...
TEST(EffectHelpers, explosion_range_basic)
{
    // level * 4 = range, for moderate levels
    ASSERT_EQ(20, static_cast<int>(compute_explosion_range(5, 0))) << "level 5 should give range 20";
    ASSERT_EQ(40, static_cast<int>(compute_explosion_range(10, 0))) << "level 10 should give range 40";
}


TEST(EffectHelpers, explosion_range_caps_at_96)
{
    ASSERT_EQ(96, static_cast<int>(compute_explosion_range(30, 0))) << "level 30 should cap at 96";
    ASSERT_EQ(96, static_cast<int>(compute_explosion_range(100, 0))) << "level 100 should cap at 96";
}


TEST(EffectHelpers, explosion_range_minimum_16)
{
    ASSERT_EQ(16, static_cast<int>(compute_explosion_range(1, 0))) << "level 1 should give minimum range 16";
    ASSERT_EQ(16, static_cast<int>(compute_explosion_range(0, 0))) << "level 0 should give minimum range 16";
}


TEST(EffectHelpers, explosion_range_skip_exit_magical)
{
    // skip_exit > 0 forces range to 0, then clamped to min 16
    ASSERT_EQ(16, static_cast<int>(compute_explosion_range(50, 1))) << "skip_exit=1 should give minimum range 16";
    ASSERT_EQ(16, static_cast<int>(compute_explosion_range(1, 1))) << "skip_exit=1 at low level should give range 16";
}
