...
void test_text_query_width_big_font_varies_by_case()
{
    text big(TEXT_BIG);
    const Sint32 wA = big.query_width("A");
    const Sint32 wa = big.query_width("a");
    TEST_ASSERT(wA > 0 && wa > 0, "query_width should be positive");
    // Uppercase path uses sizex, non-uppercase uses (sizex-1) in big-font mode.
    TEST_ASSERT(wA != wa, "uppercase and lowercase should have different widths in big-font mode");
}
REGISTER_TEST(test_text_query_width_big_font_varies_by_case);

void test_text_write_variants_smoke()
{
    text t(TEXT_1);
...
REGISTER_TEST(test_text_write_variants_smoke);

void test_text_input_string_value_accepts_textinput_and_return()
{
    text t(TEXT_1);
    ...
    TEST_ASSERT(v.has_value(), "input_string_value should return a value");
    if (v.has_value())
        TEST_ASSERT(*v == "ab", "input_string_value should capture injected text");
}
REGISTER_TEST(test_text_input_string_value_accepts_textinput_and_return);

void test_text_input_string_value_escape_returns_nullopt()
{
    text t(TEXT_1);
