...
// the 120-tick probe row. (It also closes the hole the old
// `row.time_limit or T.time_limit_ticks` left open: a row carrying
// time_limit = 0 used to bank 0 and end the match on tick one.)
TEST_F(ModesOnslaught, time_limit_knob_overrides_the_manifest_row)
{
    ModesCtfWorld fx(kOnsLevelC);  // row carries time_limit = 120
    fx.spawn_generator(FAMILY_TENT, 0, 128, 320);
    fx.spawn_generator(FAMILY_TENT, 1, 480, 320);
    ...
    fx.tick(1);
    ASSERT_TRUE(fx.world().mode.active);
    EXPECT_EQ(3600, fx.var(kOnsTimeLimit)) << "the request beats the row";
    fx.tick(125);
