...
// that replace a living entry's positional arrays.
//
// The load-bearing test here is v1_and_v2_install_identical_descriptors:
// the whole point of v2 is that it is a spelling change, so the two forms
// of the same family must leave the registry holding the same bytes. The
// rest pins the tiers — what is fatal, what warns, what stays silent —
// because a schema whose mistakes are silent is the thing v2 exists to fix.
...
// The soldier, in both spellings, with every value shared. Two free wire
// slots so both can be installed and compared in one pass.
const char* kV1Soldier = R"(families:
  living:
    - id: v1:soldier
      wire_id: 60
      name: "SOLDIER"
...
TEST(ClasspackSchemaV2, parses_every_block)
{
    ...
    // None of the v1 arrays came along.
    ASSERT_FALSE(e.base_stats.has_value());
    ASSERT_FALSE(e.derived_bonuses.has_value());
    ASSERT_FALSE(e.special_costs.has_value());
}
...
// ---------------------------------------------------------------------------
// The point: identical installed state
// ---------------------------------------------------------------------------

TEST(ClasspackSchemaV2, v1_and_v2_install_identical_descriptors)
{
    init_all_registries();

    ClasspackData v1;
    ASSERT_TRUE(parse_classpack_yaml(kV1Soldier, v1, "v1"));
    ASSERT_EQ(og::resources::install_classpack_data(std::move(v1)), 1);

    ClasspackData v2;
    ASSERT_TRUE(parse_classpack_yaml(kV2Soldier, v2, "v2"));
    ASSERT_EQ(og::resources::install_classpack_data(std::move(v2)), 1);

    const FamilyDescriptor* a = get_family_descriptor(60);
    const FamilyDescriptor* b = get_family_descriptor(61);
    ASSERT_NE(a, nullptr);
    ASSERT_NE(b, nullptr);

    for (int i = 0; i < StatAxis::Count; i++) {
        ASSERT_EQ(a->base_stats[i], b->base_stats[i]) << "base_stats " << i;
        ASSERT_EQ(a->stat_costs[i], b->stat_costs[i]) << "stat_costs " << i;
    }
    ASSERT_EQ(a->hiring_cost, b->hiring_cost);
    ASSERT_EQ(a->combat.hp, b->combat.hp);
    ASSERT_EQ(a->combat.melee_damage, b->combat.melee_damage);
    ASSERT_EQ(a->combat.stepsize, b->combat.stepsize);
    ASSERT_EQ(a->combat.fire_delay, b->combat.fire_delay);
    ASSERT_EQ(a->combat.fire_mp_cost, b->combat.fire_mp_cost);
    for (int i = 0; i < FD_NUM_SPECIALS; i++) {
        ASSERT_EQ(a->special_cost[i], b->special_cost[i]) << "cost " << i;
        ASSERT_STREQ(a->special_names[i], b->special_names[i]) << i;
        ASSERT_STREQ(a->alternate_names[i], b->alternate_names[i]) << i;
    }

    // ...and v2 additionally carries the ids the v1 arrays could not spell.
    ASSERT_EQ(a->special_ids[1], nullptr) << "v1 declares no ids";
    ASSERT_STREQ(b->special_ids[1], "charge");
    ASSERT_STREQ(b->special_ids[4], "disarm");
    ASSERT_EQ(b->special_ids[0], nullptr) << "slot 0 is not a special";
    ASSERT_EQ(b->special_ids[5], nullptr) << "undeclared slot has no id";
}
...
// ---------------------------------------------------------------------------
// Dual accept, but not in one entry
// ---------------------------------------------------------------------------

TEST(ClasspackSchemaV2, v1_entries_still_parse)
{
    ClasspackData data;
    ASSERT_TRUE(parse_classpack_yaml(kV1Soldier, data, "v1"));
    ASSERT_TRUE(data.living[0].base_stats.has_value());
    ASSERT_FALSE(data.living[0].stats.has_value());
}

TEST(ClasspackSchemaV2, one_entry_may_not_mix_the_two_spellings)
{
    bool ok = true;
    const std::string said = parse_output(
        v2_entry("      hiring_cost: 250\n"
                 "      costs:\n        hire: 250\n"),
        ok);
    ASSERT_FALSE(ok) << "a half-migrated entry is unreadable, not a merge";
    EXPECT_NE(said.find("hiring_cost"), std::string::npos) << said;
    EXPECT_NE(said.find("costs"), std::string::npos) << said;

    // Both directions of the same mistake.
    ASSERT_FALSE(parses(v2_entry(
        "      special_costs: [5000, 25, 5000, 5000, 5000, 5000]\n"
        "      specials:\n        - {id: a, name: A, mp_cost: 1}\n")));
    ASSERT_FALSE(parses(v2_entry(
        "      derived_bonuses: [1, 0, 1, 0, 0, 0, 1, 1]\n"
        "      stats:\n"
        "        strength: 1\n        dexterity: 1\n"
        "        constitution: 1\n        intelligence: 1\n"
        "        armor: 1\n        level: 1\n")));

    // Two entries in one FILE may differ, though — that is how a pack
    // migrates one family at a time.
    ASSERT_TRUE(parses(std::string(kV1Soldier) +
                       "    - id: v2:soldier\n"
                       "      wire_id: 61\n"
                       "      costs:\n        hire: 250\n"));
}
