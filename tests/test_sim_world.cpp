...
OG_UNIT_TEST(test_sim_world_r15_reset_level_progress_clears_timeout_for_same_level)
{
    ...
    const og::sim::TickResult first = world.tick(fx.save, fx.enemy_freeze, 0, fx.events);
    OG_ASSERT(!first.game_ended);

    const og::sim::TickResult second = world.tick(fx.save, fx.enemy_freeze, 0, fx.events);
    OG_ASSERT(second.game_ended);

    world.reset_level_progress();
    const og::sim::TickResult restarted = world.tick(fx.save, fx.enemy_freeze, 0, fx.events);
