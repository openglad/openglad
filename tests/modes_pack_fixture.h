...
inline constexpr const char* kTestRegistrationLua =
    ...
    "  end,\n"
    "})\n"
    // on_mode_plan engine-seam probes (9800+, test_match_plan.cpp).
    // 9800 pins the inputs marshaling (plain data only — a mistyped field
    // errors the plan) and echoes the census through the plan fields, and
    // its on_mode_init pins the chained plan->init dispatch. 9801-9803 are
    ...
    "      error(\"inputs not a table\")\n"
    "    end\n"
    "    if type(inputs.teams) ~= \"table\" then\n"
    "      error(\"teams not a table\")\n"
    "    end\n"
    "    if type(inputs.flags) ~= \"table\" then\n"
    "      error(\"flags not a table\")\n"
    "    end\n"
    "    for t = 1, 4 do\n"
    "      local row = inputs.teams[t]\n"
    "      if type(row.anchors) ~= \"number\" then\n"
    "        error(\"anchors not a number\")\n"
    "      end\n"
    "      if type(row.roster) ~= \"number\" then\n"
    "        error(\"roster not a number\")\n"
    "      end\n"
    "      if type(row.npcs) ~= \"number\" then\n"
    "        error(\"npcs not a number\")\n"
    "      end\n"
    "      if type(row.generators) ~= \"number\" then\n"
    "        error(\"generators not a number\")\n"
    "      end\n"
    "    end\n"
    "    local fsum = 0\n"
    "    for k = 1, #inputs.flags do\n"
    "      local flag = inputs.flags[k]\n"
    "      if type(flag.team) ~= \"number\" then\n"
    "        error(\"flag team not a number\")\n"
    "      end\n"
    "      if type(flag.level) ~= \"number\" then\n"
    "        error(\"flag level not a number\")\n"
    "      end\n"
    "      fsum = fsum + flag.team * 100 + flag.level\n"
    "    end\n"
    "    return {\n"
    "      mode = \"PROBE\",\n"
    "      starts = true,\n"
    "      reason = \"\",\n"
    "      authored_mask = inputs.team_count,\n"
    "      active_mask = inputs.strip_troops,\n"
    "      matched = true,\n"
    "      matched_size = inputs.teams[1].anchors * 1000\n"
    "                     + inputs.teams[2].roster * 100\n"
    "                     + inputs.teams[3].npcs * 10\n"
    "                     + inputs.teams[4].generators,\n"
    "      score_limit = inputs.score_limit * 1000 + inputs.respawn_ticks,\n"
    "      seeded_squads = true,\n"
    "      teams = {\n"
    "        { active = true, fill = \"company\", count = fsum },\n"
    "        { active = true, fill = \"troops\", count = 2 },\n"
    "        { active = false, fill = \"generators\", count = 0 },\n"
    "        { active = true, fill = \"A DELIBERATELY OVERLONG LABEL\",\n"
    "          count = 4 },\n"
    "      },\n"
    "    }\n"
    "  end,\n"
    ...
    "      return\n"
    "    end\n"
    "    og.log(\"chain\", plan.mode, plan.teams[1].count)\n"
    "  end,\n"
    "})\n"
    "og.register_level_hooks(9801, {\n"
    "  on_mode_plan = function(level, inputs)\n"
    "    return { mode = \"X\", starts = false, oblist = og.oblist() }\n"
    "  end,\n"
    "})\n"
    "og.register_level_hooks(9802, {\n"
    "  on_mode_plan = function(level, inputs)\n"
    "    return { mode = \"X\", starts = false, roll = og.rand(3) }\n"
    "  end,\n"
    "})\n"
    "og.register_level_hooks(9803, {\n"
    "  on_mode_plan = function(level, inputs)\n"
    "    og.mode_set(0, 99)\n"
    "    return { mode = \"X\", starts = false }\n"
    "  end,\n"
    "})\n"
    "og.register_level_hooks(9804, {\n"
    "  on_mode_plan = function(level, inputs)\n"
    "    return nil\n"
    "  end,\n"
    "  on_mode_init = function(level, plan)\n"
    "    if plan == nil then\n"
    "      og.log(\"chain_nil\", \"nil\")\n"
    "    else\n"
    "      og.log(\"chain_nil\", \"table\")\n"
    "    end\n"
    "  end,\n"
    "})\n"
    "og.register_level_hooks(9805, {\n"
    "  on_mode_plan = function(level, inputs)\n"
    "    error(\"plan boom\")\n"
    "  end,\n"
    "  on_mode_init = function(level, plan)\n"
    "    og.log(\"chain_err\", \"init ran\")\n"
    "  end,\n"
    "})\n"
    "og.register_level_hooks(9806, {\n"
    "  on_mode_init = function(level, plan)\n"
    "    if plan == nil then\n"
    "      og.log(\"chain_none\", \"nil\")\n"
    "    else\n"
    "      og.log(\"chain_none\", \"table\")\n"
    "    end\n"
    "  end,\n"
    "})\n"
    "og.register_level_hooks(9807, {\n"
    "  on_mode_plan = function(level, inputs)\n"
    "    return {\n"
    "      mode = \"VOCAB\",\n"
    "      starts = false,\n"
    "      reason = \"vocab: will not start\",\n"
    "      teams = {\n"
    "        { active = true, fill = \"bots\", count = 5 },\n"
    "        { active = true, fill = \"matched\", count = 3 },\n"
    "        { active = true, fill = \"empty\", count = 0 },\n"
    "        { active = false },\n"
    "      },\n"
    "    }\n"
    "  end,\n"
