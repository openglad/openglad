...
TEST(PickerCosts, picker_calculate_hire_cost_clamps_to_minimums)
{
    ...
    ASSERT_TRUE(recruit != nullptr) << "recruit should exist";

    std::uint32_t cost = session.current_cost();
    ASSERT_TRUE(cost > 0) << "hire cost should be positive";
    ASSERT_TRUE(recruit->strength > 0) << "strength should be positive";
    ASSERT_TRUE(recruit->dexterity > 0) << "dexterity should be positive";
    ASSERT_TRUE(recruit->constitution > 0) << "constitution should be positive";
    ASSERT_TRUE(recruit->intelligence > 0) << "intelligence should be positive";
    ASSERT_TRUE(recruit->armor > 0) << "armor should be positive";
    ASSERT_TRUE(recruit->level >= 1) << "level should be at least 1";
}


TEST(PickerCosts, picker_calculate_train_cost_basic_and_level_upgrade_paths)
{
    auto* team0 = new guy(FAMILY_SOLDIER);
    team0->upgrade_to_level(2);
    og::runtime::current_session->myscreen_->save_data.team_list[0].reset(team0);
    ...
    ASSERT_TRUE(cost_stats > 0) << "training cost should be positive for stat increases";
