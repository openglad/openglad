...
def generate(repo_root: Path) -> str:
    ...
    for order_name, _enum, table in orders:
        ...
        if "do_special" in hook_names:
            ...
            out.append("-- String keys are the schema-v2 special ids declared")
            out.append("-- in the family YAML; registration resolves them to")
            out.append("-- slot ints (unknown or colliding keys are load errors).")
