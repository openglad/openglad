...
#   3. The mode block reports the FFA match live: MODE_ID var 7, the
#      8-slot assignment bitmap, and at least two live fighters wearing
#      distinct band bytes 16-31
...
# Two deployed heroes (soldier + archer crew) join six init bots on the
# 8-fighter arena. Keep the script-level timeout below CTest's test timeout
# so sanitizer runs still have headroom without masking real hangs.
...
if len(taken) != 8:
    print(f'FAIL: scen850 should fill 8 band slots, bitmap {bitmap} has {len(taken)}',
          file=sys.stderr)
    sys.exit(1)
for c in taken:
    if mode_vars[32 + c] == 0:
...
print(f'PASS: 300 FFA ticks on scen850, {len(taken)} band slots, '
      f'{len(alive_band)} live fighters, clean quit')
PY
