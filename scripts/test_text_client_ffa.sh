...
#   3. The mode block reports the FFA match live at the tick-150 sample:
#      MODE_ID var 7, one band slot per deployed crew member, and at least
#      two live fighters wearing distinct band bytes 16-31 (sampled
#      mid-match: with the 2002 armor-roll expectation restored the two
#      crew fighters trade real damage, and the soldier no longer survives
#      to tick 300 under seed 42)
...
if len(lines) < 6:
    print('FAIL: Expected at least 6 JSON lines, got', len(lines), file=sys.stderr)
    sys.exit(1)
...
# Lines 2 and 4: tick results — two 150-tick windows, the match stays open
results = json.loads(lines[1]).get('results', []) + json.loads(lines[3]).get('results', [])
if len(results) != 300:
    print(f'FAIL: Expected 300 tick results, got {len(results)}', file=sys.stderr)
    sys.exit(1)
if results[149].get('tick') != 150 or results[-1].get('tick') != 300:
    print(f'FAIL: Ticks should reach 150 then 300, got {results[149].get("tick")} / {results[-1].get("tick")}', file=sys.stderr)
    sys.exit(1)
if results[-1].get('game_ended'):
    print('FAIL: FFA match must still be undecided at tick 300', file=sys.stderr)
    sys.exit(1)

# Line 3: state dump at tick 150 — the FFA match is live on the fighter band
...
if len(alive_band) < 2:
    print(f'FAIL: Expected >= 2 live band fighters at tick 150, got {len(alive_band)}',
          file=sys.stderr)
    sys.exit(1)
...
# Line 5: the tick-300 state still reports the match live
final_state = json.loads(lines[4])
if final_state.get('mode', {}).get('active') is not True:
    print('FAIL: mode.active should still be true at tick 300', file=sys.stderr)
    sys.exit(1)

# Line 6: quit confirmation
quit_msg = json.loads(lines[5])
...
print(f'PASS: 300 FFA ticks on scen850, {len(taken)} band slots (crew only, '
      f'no E3 backfill), {len(alive_band)} live fighters at tick 150, clean quit')
PY
