#!/usr/bin/env bash
# Regenerate builtin/org.openglad.longseason.glad with tools/longseason_mapgen.
#
...
#
# Usage: scripts/generate_longseason_campaign.sh [output.glad]

set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
BUILD_DIR="${REPO_ROOT}/build/ci-test"
OUTPUT="${1:-builtin/org.openglad.longseason.glad}"
...
# Restage the freshly generated package into the build tree. The test
# binaries and the playtest harness read the STAGED copy under
