...
#
# This script bypasses the outdated autotools setup and compiles directly,
# mirroring the approach used by build_web.sh.
#
...
# ----------------------------------------------------------------------------
# 4. Define source files (same as build_web.sh)
# ----------------------------------------------------------------------------
...
compile_cxx() {
    ...
    [ -n "$extra_flags" ] && obj_name="${obj_name%.o}_scen.o"
    ...
    if [ ! -f "$obj" ] || [ "$SRC_DIR/$src" -nt "$obj" ]; then
        echo "  [C++] $src $extra_flags"
        $CXX "${CXXFLAGS_ARR[@]}" $extra_flags -c "$SRC_DIR/$src" -o "$obj"
    fi
}
...
# Also compile level_editor with -DOPENSCEN for the editor binary
compile_cxx "level_editor.cpp" "-DOPENSCEN"
# The editor uses glad.cpp's main too, but we need a version with OPENSCEN
compile_cxx "glad.cpp" "-DOPENSCEN"

# Build the list for openscen (same objects minus glad.o and level_editor.o, plus the _scen versions)
for obj in "${OBJ_FILES[@]}"; do
    base=$(basename "$obj")
    if [ "$base" != "glad.o" ] && [ "$base" != "level_editor.o" ]; then
        SCEN_OBJ_FILES+=("$obj")
    fi
done
...
$CXX "${OBJ_FILES[@]}" -o "$PROJECT_ROOT/openglad" $SDL_LIBS -lm -lpthread

echo "Linking openscen..."
$CXX "${SCEN_OBJ_FILES[@]}" -o "$PROJECT_ROOT/openscen" $SDL_LIBS -lm -lpthread
