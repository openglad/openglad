...
local function yell_radius(level)
  ...
  if raw < 420 then return raw end
  return 420
end
...
local function stun_total(cur_raw, add)
  if cur_raw < 0 then return cur_raw end
  local gain = (add > 0) and add or 0
  local summed = cur_raw + gain
  local capped = (summed < 150) and summed or 150
  return (capped > cur_raw) and capped or cur_raw
end
  ...
  if sp == 1 then
    ...
    for i = 1, #newlist do
      ...
      if ob then
        ...
        local tempx_clamped = (tempx > 0) and tempx or 0
        ...
        local r1 = 0
        if r1n > 0 then r1 = og.rand(r1n) end
        local r2n = tempx_clamped * 10
        local r2 = 0
        if r2n > 0 then r2 = og.rand(r2n) end
    ...
    if self:s_hitpoints() >= self:s_max_hitpoints() then return false end
    ...
    if distance > 24 then return false end
