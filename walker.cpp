...
//    xpos, ypos); //debugging memory
  ...
  collide_ob = NULL;
  dead = 1;
  screenp->myobmap->remove(this); // remove ourselves from obmap lists
...
short walker::facing(short x, short y)
{
 ...
 if (!x)
        ...
        if (x>0)
        {
          if (slope > 2414) return FACE_DOWN;
          if (slope > 414) return FACE_DOWN_RIGHT;
          if (slope > -414) return FACE_RIGHT;
          if (slope > -2414) return FACE_UP_RIGHT;
          return FACE_UP;
        }
        else
        {
          if (slope > 2414) return FACE_UP;
          if (slope > 414) return FACE_UP_LEFT;
          if (slope > -414) return FACE_LEFT;
          if (slope > -2414) return FACE_DOWN_LEFT;
          return FACE_DOWN;
        }
}
           ...
           if (user == -1) // means we are an npc
           {
                switch (facing(x, y))
                {
                  case FACE_UP:    // For cardinal directions, fail if
                        curdir = FACE_LEFT; ret1 = walk(-step, 0);
                        break;
                  case FACE_RIGHT: // we can't walk this direction
                        curdir = FACE_UP; ret1 = walk(0, (short) (-step));
                        break;
                  case FACE_DOWN:
                        curdir = FACE_RIGHT; ret1 = walk(step, 0);
                        break;
                  case FACE_LEFT:
                        curdir = FACE_DOWN; ret1 = walk(0, step);
                        break;
                         //return returnvalue;
                  case FACE_UP_RIGHT:
...
short walker::turn(short targetdir)
{
 ...
 if ( (order!=ORDER_LIVING) || (family!=FAMILY_TOWER1) )
 {
  switch (curdir)
  {
                case FACE_UP: lastx = 0; lasty = -stepsize; break;
                case FACE_UP_RIGHT: lastx = stepsize; lasty = -stepsize; break;
                case FACE_RIGHT: lastx = stepsize; lasty = 0; break;
                case FACE_DOWN_RIGHT: lastx = stepsize; lasty = stepsize; break;
                case FACE_DOWN: lastx = 0; lasty = stepsize; break;
                case FACE_DOWN_LEFT: lastx = -stepsize; lasty = stepsize; break;
                case FACE_LEFT: lastx = -stepsize; lasty = 0; break;
                case FACE_UP_LEFT: lastx = -stepsize; lasty = -stepsize; break;
                default : lastx = 0; lasty = -stepsize;
  }
 }
  cycle = 0;
  set_frame(ani[curdir][cycle]);
...
short walker::init_fire(short xdir, short ydir)
{
  ...
  if (busy > 0) return 0;  // Too busy
  if ( (order==ORDER_LIVING) && (family==FAMILY_SOLDIER) )
    if (weapons_left < 1) return 0;
...
//and the draw level code is cleaner anyway
  //if (!this) return 0;
  if (dead) {printf("drawing a dead guy!\n"); return 0;}
...
short walker::act()
{
  ...
  if (leader && leader->dead) leader = NULL;
  ...
  if (busy > 0) busy--; // This allows busy to be our FIRING delay.
  // Find new action
           ...
           if (playerteam > -1)
           {
             ...
             else
             {
               ...
               if ( (target->owner || target->lifetime) // summoned?
                 ...
                 {
                   case FAMILY_SOLDIER: strcpy(message, "SOLDIER SLAIN"); break;
                   case FAMILY_ARCHER: strcpy(message, "ARCHER DIED"); break;
                   case FAMILY_THIEF: strcpy(message, "THIEF KILLED"); break;
                   case FAMILY_ELF: strcpy(message, "ELF KILLED"); break;
                   case FAMILY_MAGE: strcpy(message, "MAGE DIED"); break;
                   case FAMILY_SKELETON: strcpy(message, "SKELETON CRUMBLED"); break;
                   case FAMILY_CLERIC: strcpy(message, "CLERIC DIED"); break;
                   case FAMILY_FIREELEMENTAL: strcpy(message, "FIRE ELEMENTAL EXTINGUISHED"); break;
                   case FAMILY_FAERIE: strcpy(message, "FAERIE POPPED"); break;
                   case FAMILY_SMALL_SLIME:
                   case FAMILY_MEDIUM_SLIME:
                   case FAMILY_SLIME:  strcpy(message, "SLIME DESTROYED"); break;
                   case FAMILY_GHOST:  strcpy(message, "GHOST VANISHED"); break;
                   case FAMILY_DRUID:  strcpy(message,"DRUID VANQUISHED"); break;
                   case FAMILY_ORC:  strcpy(message,"ORC DIED"); break;
                   default : strcpy(message, "SOMEONE DIED"); break;
                 }
                 screenp->viewob[0]->set_display_text(message, STANDARD_TEXT_TIME);
               }
             }
...
short walker::special()
{
  ...
  if (dead) {printf("Dead guy doing special!\n"); return 0;}

  // Do we have a stats object? If not, freak out and exit :)
  if (!stats) {printf("Special with no stats\n"); return 0;}
         ...
         case FAMILY_MAGE:
                switch (current_special)
                {
                 ...
                 //#ifdef REGISTERED
                  ...
                  case 4:  // Energy wave
                    newob = fire();
                    if (!newob) return 0; // failed somehow? !?!
...
//  short newx=0, newy=0;
  ...
  //   a long strip equal to the lineofsight times the size
  //   of the weapon.
...
{
  if (eater) printf("EATING A NON-TREASURE!\n");
  return 0;
}
...
{
  if (whatfamily || lifetime) printf("Should not be hitting walker::do_summon!\n");
  return NULL;
}
