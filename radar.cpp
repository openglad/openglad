...
short radar::draw()
{
  ...
  if (radarx > (sizex - xview)) radarx = (short) (sizex - xview);
  if (radary > (sizey - yview)) radary = (short) (sizey - yview);
  if (radarx < 0) radarx = 0;
  if (radary < 0) radary = 0;
          ...
          if (can_see)
          {
            switch (obfamily)
            {
              case FAMILY_GOLD_BAR:
                do_show = (short) (YELLOW + random(5)); break;
              case FAMILY_SILVER_BAR:
                do_show = (short) (GREY + random(5)); break;
              case FAMILY_DRUMSTICK:
                do_show = (short) (COLOR_BROWN + random(2)); break;
              case FAMILY_MAGIC_POTION:
              case FAMILY_INVIS_POTION:
              case FAMILY_INVULNERABLE_POTION:
              case FAMILY_FLIGHT_POTION:
                do_show = (short) (COLOR_BLUE + random(5)); break;
              default: do_show = 0; break;
            }
          }
