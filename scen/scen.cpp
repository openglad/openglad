...
//  long pos;
  ...
  {
   printf("bad pix directory\n");
   exit(1);
  }

  printf("Scenario Editor version %s\n", GLAD_VER);
...
//       printf("Memory: %ld  ", farcoreleft());
   ...
   if (mykeyboard[SCAN_L])
   {
    ...
    scentext->write_xy(52, 32, "Load [G/S] : ", DARK_BLUE, 1);
    ...
    if (mykeyboard[SCAN_S])
    {
      ...
      myscreen->buffer_to_screen(0, 0, 320, 200);
      new_scenario_name();
      scentext->write_xy(52, 32, "Loading scenario..", DARK_BLUE, 1);
      myscreen->buffer_to_screen(0, 0, 320, 200);
      remove_all_objects(myscreen);  // kill   current obs
      for (i=0; i < 60; i ++)
        myscreen->scentext[i][0] = 0;
      load_scenario(scen_name, myscreen);
   ...
   if (mykeyboard[SCAN_S])
   {
    ...
    while (mykeyboard[SCAN_S])
      dumbcount++;
    myscreen->draw_button(50, 30, 200, 40, 1, 1);
    scentext->write_xy(52, 32, "Save [G/S] ", DARK_BLUE, 1);
    myscreen->buffer_to_screen(0, 0, 320, 200);
    while ( !mykeyboard[SCAN_S] && !mykeyboard[SCAN_G] )
      dumbcount++;
    if (mykeyboard[SCAN_S]) // save scenario
    {
      scentext->write_xy(52, 32, "Saving scenario..");
      myscreen->buffer_to_screen(0, 0, 320, 200);
      // Save the map file ..
      if (!save_map_file(grid_name, myscreen) )
       printf("\nError saving map.\n");
      else
       save_scenario(scen_name, myscreen, grid_name);
      while (mykeyboard[SCAN_S])
       dumbcount++;
    ...
    {
      //gotoxy(1,23);
      scentext->write_xy(52, 32, "Saving grid..");
      myscreen->buffer_to_screen(0, 0, 320, 200);
      if (!save_map_file(grid_name, myscreen) )
       printf("\nError saving map!\n");
      while (mykeyboard[SCAN_G])
...
//  char  *newpic;
  ...
  {
    printf("Error in saving map file %s\n", filename);
    return 0;
  }
...
char some_pix(long whatback)
{
  ...
  switch (whatback)
  {
   ...
   case PIX_GRASS_DARK_1:
    switch (i)
    {
      case 0:  return PIX_GRASS_DARK_1;
      case 1:  return PIX_GRASS_DARK_2;
      case 2:  return PIX_GRASS_DARK_3;
      case 3:  return PIX_GRASS_DARK_4;
   ...
   case PIX_GRASS_DARK_B2:
     ...
     {
       case 0:
       case 1:  return PIX_GRASS_DARK_B1;
       case 2:
       case 3:
       default: return PIX_GRASS_DARK_B2;
     }
     //break;
   ...
   case PIX_GRASS_DARK_R2:
     ...
     {
       case 0:
       case 1:  return PIX_GRASS_DARK_R1;
       case 2:
       case 3:
       default: return PIX_GRASS_DARK_R2;
     }
    //break;
   case PIX_WATER1:
    switch (i)
    {
      case 0:  return PIX_WATER1;
      case 1:  return PIX_WATER2;
      case 2:  return PIX_WATER3;
   ...
   case PIX_PAVEMENT1:
    switch (random(12))
    {
      case 0:  return PIX_PAVEMENT1;
      case 1:  return PIX_PAVEMENT2;
      case 2:  return PIX_PAVEMENT3;
      default: return PIX_PAVEMENT1;
    }
    //break;
   case PIX_COBBLE_1:
    switch (random(i))
    {
      ...
      case 3:  return PIX_COBBLE_4;
      default: return PIX_COBBLE_1;
    }
    //break;
   case PIX_BOULDER_1:
    switch (random(i))
    {
      ...
      case 3:  return PIX_BOULDER_4;
      default: return PIX_BOULDER_1;
    }
    //break;
   case PIX_JAGGED_GROUND_1:
    switch (i)
    {
      case 0:  return PIX_JAGGED_GROUND_1;
      case 1:  return PIX_JAGGED_GROUND_2;
      case 2:  return PIX_JAGGED_GROUND_3;
      case 3:  return PIX_JAGGED_GROUND_4;
      default: return PIX_JAGGED_GROUND_1;
    }
   default:  return whatback;
  }
}
...
long save_scenario(char * filename, screen * master, char *gridname)
{
  ...
  // Format of a scenario object list file is: (ver. 5)
  // 3-byte header: 'FSS'
  ...
  while (head)
  {
   if (head->ob)
   {
    if (!head) return 0;  // Something wrong! Too few objects..
...
void set_name(walker  *target, screen * myscreen)
{
  ...
  //gotoxy(1,20);
  printf("Enter '.' to not change. 9 char max.\n");
  if (strlen(target->stats->name))
   printf("Object's current name: %s\n", target->stats->name);
  else
   printf("Object's current name: NOT SET\n");

  printf("    Object's new name: ");
  NORMAL_KEYBOARD(scanf("%s", newname);)
  if (strcmp(newname, ".")) // didn't type '.'
   strncpy(target->stats->name, newname, 9);

  info_box(target,myscreen);
  ...
  while (!opt_keys[SCAN_ESC])
  {
    if (opt_keys[SCAN_E]) // toggle exit mode
    {
      if (myscreen->scenario_type & SCEN_TYPE_CAN_EXIT) // already set
        ...
        myscreen->scenario_type += SCEN_TYPE_CAN_EXIT;
      myscreen->draw_box(lm, OPT_LD(2), 255, OPT_LD(2)+7, 27, 1, 1);
      if (myscreen->scenario_type & SCEN_TYPE_CAN_EXIT)
        opt_text.write_xy(lm, OPT_LD(2), "Can Always Exit (E)         : Yes", DARK_BLUE, 1);
      else
        opt_text.write_xy(lm, OPT_LD(2), "Can Always Exit (E)         : No ", DARK_BLUE, 1);
    ...
    if (opt_keys[SCAN_G]) // toggle exit mode -- generators
    {
      if (myscreen->scenario_type & SCEN_TYPE_GEN_EXIT) // already set
        ...
        myscreen->scenario_type += SCEN_TYPE_GEN_EXIT;
      myscreen->draw_box(lm, OPT_LD(3), 255, OPT_LD(3)+7, 27, 1, 1);
      if (myscreen->scenario_type & SCEN_TYPE_GEN_EXIT)
        opt_text.write_xy(lm, OPT_LD(3), " Kill Generators to Exit (G): Yes", DARK_BLUE, 1);
      else
        opt_text.write_xy(lm, OPT_LD(3), " Kill Generators to Exit (G): No ", DARK_BLUE, 1);
    ...
    if (opt_keys[SCAN_N]) // toggle fail mode -- named guys
    {
      if (myscreen->scenario_type & SCEN_TYPE_SAVE_ALL) // already set
        ...
        myscreen->scenario_type += SCEN_TYPE_SAVE_ALL;
      myscreen->draw_box(lm, OPT_LD(4), 255, OPT_LD(4)+7, 27, 1, 1);
      if (myscreen->scenario_type & SCEN_TYPE_SAVE_ALL)
        opt_text.write_xy(lm, OPT_LD(4), " Must Save Named NPC's (N)  : Yes", DARK_BLUE, 1);
      else
        opt_text.write_xy(lm, OPT_LD(4), " Must Save Named NPC's (N)  : No ", DARK_BLUE, 1);
