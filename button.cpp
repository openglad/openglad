...
long ventermenu(vbutton * vbuttons)
{
  ...
  if (vbuttons == NULL) return 0; //if no buttons, exit
  here = vbuttons;
  while(here)
  {
    here->vdisplay();
...
//  bdelta = (random(3)+1);
  ...
  while(1)
  {
         ...
         if (mykeyboard[SCAN_CTRL])
         {
                if (mykeyboard[SCAN_NUMERIC_PLUS])
                {
                  add_money((long)2);
                  return 1;
                }
                if (mykeyboard[SCAN_NUMERIC_MINUS])
                {
                  add_money((long)-2);
                  return 1;
                }
                if (mykeyboard[SCAN_OPEN_BRACKET]) // lower scen num
                {
                  if (scen_level > 1)
                         scen_level--;
                ...
                if (mykeyboard[SCAN_CLOSE_BRACKET]) // raise scen num
                {
                  scen_level++;
...
//              input = temp;
         ...
         if (mykeyboard[SCAN_ENTER])
         {
                while (mykeyboard[SCAN_ENTER])
                  dumbcount++;
                if (!no_clear)
                  clearmenu(buttons,numbuttons);
                if (buttons[buttonnum].fun)
