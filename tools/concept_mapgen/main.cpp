 ...
 *
 * Produces builtin/org.openglad.concept.glad (levels 600-604): a handful of
 * small multi-floor scenarios that show off the Z-axis feature — stacked floors
 * joined by Z-stairs, "air" holes you fall through, see-through "glass" floors,
 * and projectile arcs. SDL-free; reuses the headless platform glue, mirrors
 * tools/ctf_mapgen. Builds the v10 multi-floor scenario format
...
struct ExpectedLevel { int id; int floors; const char* title; };
    ...
    if (g_errors == 0)
    {
        ...
        else
        {
            const ExpectedLevel expectations[] = {
                {600, 2, "Stairs"}, {601, 2, "Mind the Gap"},
                {602, 2, "Glasshouse"}, {603, 2, "Drop Zone"},
                {604, 2, "Arc Range"},
            };
