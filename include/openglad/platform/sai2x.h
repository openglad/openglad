...
class Screen
{
	public:
		...
		// never presented. `detail` receives where it differs — the bounding
		// box of the changed pixels, or "never presented" — and is cleared
		// on the matching path. "Showed" means the rect each present
		// DECLARED: a partial-rect swap() records only that rect, so a draw
		// outside it stays unpresented until a present covers it.
		bool testing_render_matches_presented(std::string& detail) const;
