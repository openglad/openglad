...
| D12 | **Parity/canary wording** | Correct acceptance wording everywhere: "all PRE-EXISTING og_test_parity rows/tests pass byte-identically; the scenario table grows by exactly one Invariant row (fall damage), mutation-less exempt Invariant rows go 4 → 5, and walker.cpp pin 2173 is re-pinned once." Claims of "187/187 unchanged" are void once WP-1 lands. **Update (2026-09-17, PR #292):** the "exempt Invariant rows" tally no longer exists — branch-internal rows are excluded from every canary mode by design (`.claude/skills/openglad-parity/SKILL.md`, "The mutation canary") — so the canary half of this acceptance wording is now "`scripts/parity/run_mutation_canary.sh` exits 0 with `zero_flips=0 predicate_toothless=0`", the two tallies that must hold. |
| Errata | **Pin-map corrections** | `save_data.cpp` canary pin is line **118** (scenario_table.h:1179) — designs citing 107 were wrong. `game_world.cpp` has THREE pins: **1620, 1622, 1710** (scenario_table.h:1193/1165/3291) — the HUD spec's "1620/1622 only" was wrong; its EOF append is still safe (appends shift nothing). walker.cpp pins: **1192×8, 1354, 2173** (2173 @ scenario_table.h:4805). walker_specials.cpp, screen.cpp, results_screen.cpp, score_panel.cpp, radar.cpp, fps_overlay.cpp, local_transport_shadow.cpp, headless_server_runtime.cpp, curses_*.cpp, picker_*.cpp, campaign_yaml.cpp, campaign_metadata.cpp carry NO pins. **Update (2026-09-17, PR #292):** `save_data.cpp` carries no canary pin at all — `kMut_save_corrupt` was deleted as an orphan pin (tests/parity/golden/DRIFT_LEDGER.md, "Removed pins (2026-09-15)"). Every other line number in this row is the fd097693 snapshot; the live map is `grep -n '"src/' tests/parity/scenario_table.h`, as the line-3 warning already says. |
...
7. **Save format append-only.** GTL v12 → v13 exactly once (WP-5), all edits below the save_data.cpp:118 pin, both format-comment blocks (:143-194, :736-787) updated, test_save_data_versions.cpp writer branch + roundtrip + defaults.
   **Update (2026-09-17, PR #292):** the ":118 pin" is gone — `save_data.cpp` carries no canary pin any more (`kMut_save_corrupt` was deleted as an orphan pin; tests/parity/golden/DRIFT_LEDGER.md, "Removed pins (2026-09-15)"), so the append-only rule stands on its own and `scripts/parity/check_mutation_pins.py` has nothing to guard in that file. The live pin map is `grep -n '"src/' tests/parity/scenario_table.h`.
...
- **Canary baseline:** mutation-less exempt Invariant rows go 4 → 5; `run_mutation_canary.sh --all` still exits 1 by design; **genuine toothless must stay 0**.
  **Update (2026-09-17, PR #292):** both halves of this baseline are void. Branch-internal rows are excluded from every canary mode, so no run reports exempt Invariant rows; `run_mutation_canary.sh --all` exits 0 on a healthy tree (first measured at `ed0076cb`: `rows=222 groups=197 rebuilds=64 zero_flips=0 predicate_toothless=0`, tests/parity/golden/DRIFT_LEDGER.md, "The full `--all` re-measure") and 1 is a real teeth failure. The two tallies that must stay 0 are `zero_flips` and `predicate_toothless`, and the canary runs in CI (`.github/workflows/parity-canary.yml`).
...
- Row drawn AFTER the wave block (:630) at `tm + (show_wave ? 26 : 18)` (drawn once at floor_y; the 2013 `+44+8` touch variant and its `#ifndef USE_TOUCH_INPUT` fork were retired in PR #292), at the counter box's one shared text column, color `text_color`; add `TRACE("hud", "floor %s", floor_label.c_str())` (the shared label, never the fitted text). PR #292 replaced the per-row right-align idiom with a single fit rule in `include/openglad/interface/hud_counter_box.h`: the box's width is derived from its widest row (`left = min(rm-57, rm-4-6*widest)`, floored at `min(lm+66, rm-57)`), every row is left-aligned at `left+2`, and a row that overruns the capacity drops its label before it is truncated. So `FLR: 10/12` now widens the box to rm-64 instead of overflowing it, and the wave row is `WAVE: {s}s`. Single-floor frames are pixel-identical (label empty), and so is every frame whose rows all fit 8 glyphs.
...
- tests/integration/test_save_data_versions.cpp (:79-211): v13 synthetic-writer branch + roundtrip pinning the version byte + old-save (v12) load-defaults test.
...
**Update (2026-09-17, PR #292):** "manual canary run" is the parity-canary CI lane now — `.github/workflows/parity-canary.yml` runs `--touched <merge-base>` on every pull request and `--all` nightly / on `workflow_dispatch`; nothing in this gate list is run by hand.
...
3. **Mutation canary (manual, not CI):** clean worktree; dry-run `_apply_mutation.py` on all walker.cpp pins after WP-1's repin (sed-verify first); spot canary run on walker.cpp; expected: 5 mutation-less exempt Invariant rows, `--all` exits 1 by design, **genuine toothless = 0** (`grep -c '0 flips (mutation' log`). Before EVERY commit in any WP: `grep -n '"src/' tests/parity/scenario_table.h` and confirm no pinned file's pinned line shifted (pins: walker.cpp 1192×8/1354/2173+Δ; living.cpp 114; walker_combat.cpp 99/209/302; game_world.cpp 1620/1622/1710; save_data.cpp 118).
   **Update (2026-09-17, PR #292):** the canary is a CI lane (`.github/workflows/parity-canary.yml`: `--touched` per PR, `--all` nightly), so "manual, not CI" no longer holds; `--all` exits 0 on a healthy tree and 1 on any zero-flip or PREDICATE-TOOTHLESS row, and it reports no exempt-Invariant tally (branch-internal rows are excluded from every mode). The pin map in this item is the fd097693 snapshot — re-grep it as the item already says (`grep -n '"src/' tests/parity/scenario_table.h`) — and `save_data.cpp` carries NO canary pin at all: `kMut_save_corrupt` was deleted as an orphan (tests/parity/golden/DRIFT_LEDGER.md, "Removed pins (2026-09-15)").
...
| R12 | Non-replicated fall accumulator on mirrors (late joiner under-counts a mid-cascade fall) | LOW | Accepted z_stair_latched_ contract; hp self-corrects on next snapshot; documented in the field comment. The floating number is lifted onto the sim event batch by GameServer (`EventKind::DamageNumber`) and does reach every mirror; a late joiner mid-cascade still misses it. |
