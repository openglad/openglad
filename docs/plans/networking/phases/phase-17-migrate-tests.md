...
All integration tests that currently call `game_frame()` or `screen::act()` are migrated to use the `NetworkTestFixture` (built in Phase 13). The fixture creates a GameServer + GameClient with InProcessTransport, which exercises the real networking code path. This means every migrated integration test validates the server/client architecture as a side effect.
