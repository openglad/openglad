...
**Dirty tracking instrumentation is deferred to [Phase 8](phase-08-setter-dirty-tracking.md).** The infrastructure (`dirty_mask_[2]`, `mark_dirty()`, `mark_all_dirty()`, `clear_dirty()`, bit constants in `dirty_field_bits.h`) exists from Phase 2, and the cross-reference setters already call `mark_dirty()`. But the large-scale instrumentation of all ~86 remaining serializable fields is deferred to Phase 8 where the setter refactor makes all fields private and adds getter/setter pairs that call `mark_dirty()`. Until then, keyframe captures set all bits (all fields dirty), so the absence of per-field `mark_dirty()` calls has no effect. This keeps Phase 3 focused on a single concern (pointer privatization) and ensures the instrumentation and its CI safety-net test land in the same phase.
