# Phase 16: Split Legacy Gameplay Wrapper Into Sub-Methods (Pure Refactor)
...
Extract the internals of the legacy gameplay wrapper into three new methods, leaving a thin compatibility wrapper that calls them in sequence. **All ~1787 existing tests pass unchanged** — this is a behavior-identical refactor.

**Gameplay-wrapper refactoring:** The current wrapper does: `world_.tick()` -> dispatch events (8 early return paths identified: lines 903, 955, 1017, 1031, 1036, 1043, 1066, 1068). Split into three methods:
...
- The wrapper has **8 early return paths** (line 903: null context, 955: EndGame, 1017: withdraw load error, 1031: withdraw save error, 1036: withdraw success, 1043: exit success, 1066: world.end, 1068: normal exit). These all need to map to game-flow events or snapshot flags rather than immediate returns.
...
**Compatibility wrapper:** After the split, the wrapper becomes conceptually:
```cpp
bool gameplay_wrapper() {
...
All existing tests and the game loop call the compatibility wrapper exactly as before. The sub-methods are new public API that Phase 17 will use.
