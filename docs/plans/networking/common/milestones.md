...
- **Phase 16** — legacy gameplay wrapper split into sub-methods. Pure refactor, all tests pass unchanged.
...
- **Phase 18** — integration tests calling `game_frame()` or the legacy gameplay wrapper migrated to server/client path. The wrapper is deleted. No fallback.
