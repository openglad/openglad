...
Phase 10 replaced the rough estimate with a real worst-case benchmark on the built-in campaign: `org.openglad.gladiator` level 30, `LORD JAKARTA'S GATES (HARD!)`, with a scripted 4-player ArchMage/Archer/Thief/Cleric team after 100 ticks of combat. That scenario consistently hit **426 entities total** (`oblist=131`, `fxlist=269`, `weaplist=26`) with `sizeof(EntitySnapshot) == 280` bytes. The benchmark prints both the compressed **zlib payload** size and the full **wire message** size (`zlib payload + 8-byte transport header`). The uncompressed raw payload sizes are stable; the compressed full-keyframe result has shown enough clean-build variation that the planning numbers below intentionally round upward to a conservative on-wire budget:

- Full keyframe snapshot: **118,547-byte raw payload**, budget as **~17KB on wire**
- 1-tick transport delta: **11,248-byte raw payload**, budget as **~3.0KB on wire**
- 10-tick accumulated catch-up delta: **13,053-byte raw payload**, budget as **~3.7KB on wire**

For bandwidth math, use the **1-tick delta** and use the **on-wire message size**, not the 10-tick catch-up packet and not the raw payload size. At default speed (`timer_wait = 6`, ~12 ticks/sec), a sustained delta stream at the measured worst-case size is about **36KB/sec per client** on the wire, or about **144KB/sec outbound** for 4 clients before adding keyframes. Periodic full keyframes add about **3.4KB/sec per client** on the wire at default speed, so snapshot traffic alone is roughly **158KB/sec outbound** in this peak-load scenario. The 10-tick catch-up packet is still useful as a skipped-send/recovery datapoint, but it should not be multiplied by ticks/sec. Even with the corrected math, the measured peak is materially higher than the original 40KB/8-16KB/1-3KB estimate, but still well within residential internet capacity.
...
**SimEventBatch overhead:** The numbers above only cover snapshot/delta traffic. `SimEventBatch` messages (sounds, notifications, palette changes) still add roughly ~10-30KB/sec during active combat (~61 cosmetic event call sites across gameplay code — 23 PlaySound + 35 Notification + 2 SetPalette + 1 RequestRedraw). That pushes the measured default-speed peak closer to **~168-188KB/sec outbound**. At max networked speed (`timer_wait = 1`, ~73 ticks/sec), bandwidth scales roughly linearly, so worst-case outbound can reach **~1.0-1.2MB/sec**. That is still serviceable, but it narrows the safety margin enough that later transport phases should treat the 400+ entity benchmark as the planning baseline, not the older 200-entity estimate.
