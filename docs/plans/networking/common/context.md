...
Phase 10 replaced the rough estimate with a real worst-case benchmark on the built-in campaign: `org.openglad.gladiator` level 30, `LORD JAKARTA'S GATES (HARD!)`, with a scripted 4-player ArchMage/Archer/Thief/Cleric team after 100 ticks of combat plus a 10-tick accumulated delta window. Repeated runs produced a stable peak of **426 entities total** (`oblist=131`, `fxlist=269`, `weaplist=26`) and `sizeof(EntitySnapshot) == 280` bytes. The measured wire payload sizes were:

- Full keyframe snapshot: **118,547 bytes raw**, **~15.5KB after zlib** (`15,510-15,518 bytes` across repeated runs)
- 10-tick accumulated delta: **13,053 bytes raw**, **~3.7KB after zlib** (`3,659 bytes` across repeated runs)

At default speed (`timer_wait = 6`, ~12 ticks/sec), a sustained delta stream at the measured worst-case size is about **44KB/sec per client** (`3,659 x 12`), or about **176KB/sec outbound** for 4 clients before adding keyframes. Periodic full keyframes add about **3.1KB/sec per client** at default speed (`~15.5KB / 5s`), so snapshot traffic alone is roughly **188KB/sec outbound** in this peak-load scenario. This is materially higher than the original 40KB/8-16KB/1-3KB estimate, but still well within residential internet capacity.
...
**SimEventBatch overhead:** The numbers above only cover snapshot/delta traffic. `SimEventBatch` messages (sounds, notifications, palette changes) still add roughly ~10-30KB/sec during active combat (~61 cosmetic event call sites across gameplay code — 23 PlaySound + 35 Notification + 2 SetPalette + 1 RequestRedraw). That pushes the measured default-speed peak closer to **~198-218KB/sec outbound**. At max networked speed (`timer_wait = 1`, ~73 ticks/sec), bandwidth scales roughly linearly, so worst-case outbound can reach **~1.2-1.3MB/sec**. That is still serviceable, but it narrows the safety margin enough that later transport phases should treat the 400+ entity benchmark as the planning baseline, not the older 200-entity estimate.
