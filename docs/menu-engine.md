...
| Main menu | **Engine** | `main_menu_screen_spec()` via `mainmenu()` — one centered primary stack shared by the §1.6 MP/no-MP spec pair; a labeled 2+1 SETTINGS group (PLAYER / DIFFICULTY over GAME SETTINGS); stable HELP / QUIT footer with build-gated enabled/disabled QUIT; `FadeWithInitialDraw` entry, MainScope remote-start with `BreakWithSelection`, and the G14 full-re-vdisplay content hook |
