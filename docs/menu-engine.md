...
| PLAYER SETTINGS | **Engine** | `player_settings_menu_screen_spec()` from the main-menu door — local player-count selector, PVP relationship, and the CONTROLS door; multiplayer-disabled builds expose only CONTROLS |
...
| MAIN OPTIONS | **Engine** | `main_options_menu_screen_spec()` via `main_options()` — sound, display, and effects; keeps the family-wide cfg-persist exit epilogue |
