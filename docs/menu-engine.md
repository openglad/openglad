...
3. **The web/native fork is positional, not just row-gated.** The two
   ...
   indices agree across variants (the main-menu QUIT-state precedent); G9
...
- **Crossed bottom-row down-links, historical native main menu** (the
  pre-PLAYER SETTINGS `picker.cpp` k_mainmenu_buttons): `pvp_allied`
  (center x=114) has `.down=9` → quit
...
- **Exact ties in that historical table**: `continue_game` → `1_player`
  ties `2_player` at 36px; the hand-wired pick encoded leftmost-first intent.
...
- `MenuBuildGate` filtering shifts materialized indices; the runner and the
  ...
  across variants (the main-menu enabled/disabled-QUIT fork satisfies this
  by construction — one survivor, same index, same links).
- The `change_allied` raw `allbuttons_[7]` click-side write was DELETED by
  ...
  with its test_picker_funcs re-pin. The `pvp_allied` LabelBinding now lives
  in PLAYER SETTINGS and still re-derives both label surfaces every frame.
