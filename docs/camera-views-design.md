...
- Geometry (pinned), **2 seats — near-minimap ×2 (maintainer ruling)**: each of the two seats gets the one-seat second-minimap block of the next bullet, computed from ITS OWN UI pane — `compute_view_layout(2, i, seat PREF_VIEW, ui_w, ui_h)` for seat `i`, the side-by-side split — and stacked directly above its own radar block with the radar's footprint and margin; both blocks show the same camera target at the same `kCameraMinimapZoomDenominator` zoom. At the classic canvas over the scen820 pitch that is `(111, 136, 44, 28)` above seat 0's radar `(111, 168, 44, 28)` and `(272, 136, 44, 28)` above seat 1's radar `(272, 168, 44, 28)`. The camera viewscreen stays singular (one `camera_view_`, one replicated slot); the multiplicity is draw geometry only — `screen::camera_pane_rects_` (count 1 or 2) is what every inset path reads: `draw_camera_view_ui` renders the zoomed layer ONCE and samples it into every block through one `camera_scale_end` per rect when the blocks share extents (they do for the symmetric split; differing extents fall back to per-block rendering), the border draws per rect, the allocation-failure 1:1 fallback draws per rect, the stale-pixel scrub scrubs EVERY previous rect on a structural transition, and the R2 overlay-absent yield covers all of them. `camera_docked_` / `layout_pane_count()` are untouched (still false / `== numviews`), no present slices, no world-canvas perturbation. The style rule — 1 seat → minimap, 2 seats → minimap ×2, 4 seats → centered — is ONE `switch` in `relayout_camera_view` (`camera_minimap_block_for_seat` is the shared per-seat geometry, applied to seat 0 at 1 seat and to both seats at 2).
- Geometry (pinned), **1 seat — the SECOND MINIMAP (maintainer ruling)**: with one seat the canvas centre is exactly where the seat camera holds the player's own hero, so a centred pane covers him. The camera becomes a second minimap instead: **the radar block mirrored** — the same width and height, the same right-edge alignment, stacked **directly above** the radar's rect with the radar's own pane margin between the two. Derived, not typed: `radar_block_extents(grid_w, grid_h)` gives the block size (the `RADAR_X × RADAR_Y` viewport clamped to the grid, exactly as the live radar clamps it) and `radar_block_for_pane(...)` gives its placement inside the seat's UI pane — the SAME two functions `radar::sync_to_grid`/`radar::sync_position_to_view` place the minimap with, so the two can never drift ("no rule twins"). The pane rect comes from `compute_view_layout(layout_pane_count(), 0, seat PREF_VIEW, ui_w, ui_h)` — the projection `ScopedGameplayUiViewLayout` applies before the radar draws — never a hardcoded 320×200. The anchor is the **computed** radar position, so `PREF_RADAR` off (no minimap drawn at all) places the camera identically. At the classic canvas with a 44×28-tile pitch (scen820) that is `(272, 136, 44, 28)`, with the radar block at `(272, 168, 44, 28)`. **And the pane draws at 0.25 zoom (maintainer ruling):** a radar-sized rect at 1:1 is too zoomed-in to be useful, so the pane keeps its exact rect but shows a `kCameraMinimapZoomDenominator`(=4)-times wider and taller world window around the target, downscaled 4:1. That is roughly 11×7 tiles in the scen820 pane — a quarter of the pitch rather than a keyhole. **The price, stated plainly:** at 4:1 a 32×32-pixel walker sprite arrives as an 8×8 block and a 16×16 ball as ~4px, so the pane reads as coloured markers moving over terrain, not as legible sprites — which is the right register for a *minimap*, and the reason the same zoom is not applied to the full-size docked quadrant or the 4-seat inset (the 2-seat blocks are minimaps too and take it). Mechanism: `video::camera_scale_begin/end` — the floor_layer redirect shape on its OWN off-screen surface (so a multi-floor camera redraw can still nest floor layers inside it): `draw_camera_view_ui` renders the widened window 1:1 into the layer through the SAME `viewscreen::redraw` data overload (the viewscreen is simply resized to 4w×4h for the draw, so the seat centering and world-edge behavior are inherited, never re-implemented), then the backend copies the layer onto the pane rect with an integer nearest sample — `dst(i,j) = layer(i*4, j*4)`, whole-pixel copies, no floats, no filtering. A failed layer allocation yields to the 1:1 draw for the frame (the R2 degrade shape); the R1 overlay suppression, the border, and the stale-pixel scrub ride the unchanged pane rect. The docked quadrant and the 4-seat centered inset are full-size panes and stay 1:1.
