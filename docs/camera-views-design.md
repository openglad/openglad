...
| Inset geometry (1 seat) | **A second minimap**: the radar block mirrored — same size, same right edge, stacked one radar margin above the radar's rect | Maintainer ruling (a centered pane at 1 seat sits exactly on the player's own hero, because the seat camera centres him) |
...
- Geometry (pinned), **1 seat — the SECOND MINIMAP (maintainer ruling)**: with one seat the canvas centre is exactly where the seat camera holds the player's own hero, so a centred pane covers him. The camera becomes a second minimap instead: **the radar block mirrored** — the same width and height, the same right-edge alignment, stacked **directly above** the radar's rect with the radar's own pane margin between the two. Derived, not typed: `radar_block_extents(grid_w, grid_h)` gives the block size (the `RADAR_X × RADAR_Y` viewport clamped to the grid, exactly as the live radar clamps it) and `radar_block_for_pane(...)` gives its placement inside the seat's UI pane — the SAME two functions `radar::sync_to_grid`/`radar::sync_position_to_view` place the minimap with, so the two can never drift ("no rule twins"). The pane rect comes from `compute_view_layout(layout_pane_count(), 0, seat PREF_VIEW, ui_w, ui_h)` — the projection `ScopedGameplayUiViewLayout` applies before the radar draws — never a hardcoded 320×200. The anchor is the **computed** radar position, so `PREF_RADAR` off (no minimap drawn at all) places the camera identically. At the classic canvas with a 44×28-tile pitch (scen820) that is `(272, 136, 44, 28)`, with the radar block at `(272, 168, 44, 28)`.
