...
| Inset geometry (1 seat) | **A second minimap**: the radar block mirrored — same size, same right edge, stacked one radar margin above the radar's rect — drawn at **0.5 zoom** (`kCameraMinimapZoomDenominator = 2`): a 2x-wider/taller world window integer-downsampled 2:1 through the off-screen `camera_scale` layer | Maintainer ruling (a centered pane at 1 seat sits exactly on the player's own hero, because the seat camera centres him; and the radar-sized rect at 1:1 is too zoomed-in to be useful) |
...
- Geometry (pinned), **1 seat — the SECOND MINIMAP (maintainer ruling)**: with one seat the canvas centre is exactly where the seat camera holds the player's own hero, so a centred pane covers him. The camera becomes a second minimap instead: **the radar block mirrored** — the same width and height, the same right-edge alignment, stacked **directly above** the radar's rect with the radar's own pane margin between the two. Derived, not typed: `radar_block_extents(grid_w, grid_h)` gives the block size (the `RADAR_X × RADAR_Y` viewport clamped to the grid, exactly as the live radar clamps it) and `radar_block_for_pane(...)` gives its placement inside the seat's UI pane — the SAME two functions `radar::sync_to_grid`/`radar::sync_position_to_view` place the minimap with, so the two can never drift ("no rule twins"). The pane rect comes from `compute_view_layout(layout_pane_count(), 0, seat PREF_VIEW, ui_w, ui_h)` — the projection `ScopedGameplayUiViewLayout` applies before the radar draws — never a hardcoded 320×200. The anchor is the **computed** radar position, so `PREF_RADAR` off (no minimap drawn at all) places the camera identically. At the classic canvas with a 44×28-tile pitch (scen820) that is `(272, 136, 44, 28)`, with the radar block at `(272, 168, 44, 28)`. **And the pane draws at 0.5 zoom (maintainer ruling):** a radar-sized rect at 1:1 is too zoomed-in to be useful, so the pane keeps its exact rect but shows a `kCameraMinimapZoomDenominator`(=2)-times wider and taller world window around the target, downscaled 2:1. Mechanism: `video::camera_scale_begin/end` — the floor_layer redirect shape on its OWN off-screen surface (so a multi-floor camera redraw can still nest floor layers inside it): `draw_camera_view_ui` renders the doubled window 1:1 into the layer through the SAME `viewscreen::redraw` data overload (the viewscreen is simply resized to 2w×2h for the draw, so the seat centering and world-edge behavior are inherited, never re-implemented), then the backend copies the layer onto the pane rect with an integer nearest sample — `dst(i,j) = layer(i*2, j*2)`, whole-pixel copies, no floats, no filtering. A failed layer allocation yields to the 1:1 draw for the frame (the R2 degrade shape); the R1 overlay suppression, the border, and the stale-pixel scrub ride the unchanged pane rect. The docked quadrant and the 2/4-seat centered inset are full-size panes and stay 1:1.
