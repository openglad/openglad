...
| Inset geometry (1 seat) | **A second minimap**: a 96×60 live-art pane sharing the radar's right edge, stacked one radar margin above it, drawn at **0.5 scale** (`kCameraMinimapZoomDenominator = 2`): a 2×-wider/taller world window projected directly into the final pane raster | Maintainer correction after the shipped basketball review: the old 45×25 grid-clamped, quarter-scale pane wasted available pixels and reduced the ball to a speck |
| Inset geometry (2 seats) | **Near-minimap ×2**: the one-seat live-art block for each seat, right-aligned above that seat's radar inside its side-by-side pane, with the same 0.5 scale; one camera viewscreen, two draw rects (`camera_pane_rects_`), each projected directly into its final pane raster. Height only contracts on a short pane when needed to stay below the seat centre | Maintainer ruling (replaces the centered 1:1 inset at 2 seats; 4 seats keep it) |
...
- Geometry (pinned), **2 seats — near-minimap ×2 (maintainer ruling)**: each seat gets a live-art block from its own UI pane, computed with `compute_view_layout(2, i, seat PREF_VIEW, ui_w, ui_h)` and right-aligned above that seat's radar. Both blocks show the same target with `kCameraMinimapZoomDenominator` projection. At the classic canvas over scen820, the blocks are `(59, 104, 96, 60)` and `(220, 104, 96, 60)`. If a short pane lifts its radar toward the seat centre, only the camera height contracts enough to remain below that centre. `screen::camera_pane_rects_` holds the draw geometry; `draw_camera_view_ui` renders each final pane directly. The border and stale-pixel scrub still visit every rect. `camera_docked_`, `layout_pane_count()`, present slices, and the world canvas are unchanged. One `switch` in `relayout_camera_view` owns the style rule: one seat gets one minimap, two seats get two minimaps, and four seats get the centered inset.
- Geometry (pinned), **1 seat — the SECOND MINIMAP (maintainer ruling)**: the camera is a 96×60 live-art pane right-aligned above the radar; `radar_block_for_pane(...)` remains the single placement anchor, but the radar's one-pixel-per-cell size clamp no longer dictates the camera raster. At the classic canvas over scen820, the camera is `(220, 104, 96, 60)` and the radar is `(272, 168, 44, 28)`; on scen824 the 45×25 radar likewise gets a full 96×60 camera. The pane shows a world window twice as wide and tall. `viewscreen::render_denominator_` enlarges the world window without enlarging `xview` or `yview`; tile bounds use the world-window dimensions, and tiles, decor, walkers, shadows, reflections, particles, glows, floor masks, and weather project into final pane coordinates. Indexed images use `video::putbuffer_projected`, whose loop visits destination pixels and samples the source image for each pixel. No full-scene scratch surface or camera downsample API exists. Docked and four-seat panes use denominator 1 and retain the classic arithmetic.
