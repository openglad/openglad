...
| Inset geometry (1 seat) | **A second minimap**: the radar's compact footprint mirrored one radar margin above it; its live scene uses the renderer-wide native-world plane at physical-output density | Maintainer correction after the basketball review: zoom the content, not the HUD rectangle; CENTER COURT stays 45×25 logical UI units while every physical pixel supplies world detail |
| Inset geometry (2 seats) | **Near-minimap ×2**: the one-seat compact block for each seat, mirrored above that seat's radar inside its side-by-side pane; one native source is presented into both draw rects (`camera_pane_rects_`) | Maintainer ruling (replaces the centered inset at 2 seats; 4 seats keep it) |
...
- Geometry (pinned), **2 seats — near-minimap ×2 (maintainer ruling)**: each seat gets a live-art block from its own UI pane, computed with `compute_view_layout(2, i, seat PREF_VIEW, ui_w, ui_h)`, with exactly that seat's radar width and height and mirrored one radar margin above it. At the classic canvas over scen820, the blocks are `(111, 136, 44, 28)` and `(272, 136, 44, 28)`. `screen::camera_pane_rects_` holds both destinations; `draw_camera_view_ui` renders one physical-density native-world source and queues it for both. The border and stale-pixel scrub still visit every rect. `camera_docked_`, `layout_pane_count()`, present slices, and the world canvas are unchanged. One `switch` in `relayout_camera_view` owns the style rule: one seat gets one minimap, two seats get two minimaps, and four seats get the centered inset.
- Geometry (pinned), **1 seat — the SECOND MINIMAP (maintainer ruling)**: with one seat the canvas centre is exactly where the seat camera holds the player's own hero, so a centred pane would cover him. The camera therefore mirrors the radar block: same width, height, and x coordinate, stacked one radar margin above it. `radar_block_for_pane(...)` is the single placement anchor. At the classic canvas over scen820, the camera is `(272, 136, 44, 28)` above the radar `(272, 168, 44, 28)`; CENTER COURT is `(271, 142, 45, 25)` above its 45×25 radar. That logical rectangle is only placement: at a 640×400 output its scene is a 90×50 world raster; on a 2×-density 640×400 window it is 180×100. The backend derives that source size from the physical destination, providing the zoom-out without enlarging the HUD block or teaching individual sprite/effect paths how to project.
- **Draw order — above HUD chrome, below pause UI.** Both seams must be patched (`hud` recon):
  ...
  At each seam: `{ ScopedGameplayUiCanvas ui(s); s.draw_camera_view_ui(); }`. `draw_camera_view_ui()` gives the backend its GameplayUI destinations; the backend derives one source pixel per physical output pixel and redirects the ordinary data-overload world redraw into that raster. The draw restores the camera geometry and paints only the 1px border into GameplayUI. `Screen::swap` composites all native-world planes after the base canvas and HUD. The picker staged preview uses the same generic plane with a UI destination; neither caller writes live world pixels into a fixed canvas.
...
- Per-walker UI overlays — mini HP bars and damage/heal numbers (code-review finding R1): a camera pane draws NONE of them. Both blocks in `walker_draw.cpp` are gated on the pane's explicit `camera_view_` flag (the chrome-scope mechanism, never the `mynum==-1` clamp coincidence), because their GameplayUI projection is keyed on `layout_pane_count()` + `mynum` and has no camera arm — a 1-seat inset would scale them across the full UI canvas and 4 seats would stamp them into seat 3's quadrant. (Spectator and follow panes are seat panes for this purpose and DO paint numbers — for whichever walker the pane's `control` watches, seat-bound or not; the rule here is about `camera_view_` insets only.)
...
| WP4 | **Inset render at the seams** — `draw_camera_view_ui` through the renderer-wide native-world plane, both game-loop seams, border, allocation-failure answer + physical-output probe | **HARD** (new offscreen render and final-composite path shared by every embedded live-world view) | WP3 | integration tests 4, 10; native-source/destination pins; physical sample-density proof; visual capture; no present-slice deltas |
...
1. **Native-world plane allocation or presentation fails** — the inset is omitted for that frame and the fixed UI is left untouched; there is no per-object reduced-resolution fallback. Detected by the allocation-failure pixel test, the physical-output sample test, the inset integration test, and mandatory visual capture.
