...
| Inset geometry (1 seat) | **A second minimap**: the radar's compact footprint mirrored one radar margin above it, with the followed world rendered **1:1** inside that final pane | Maintainer correction after the basketball review: zoom the content, not the HUD rectangle; CENTER COURT stays 45×25 while its ball retains full sprite scale |
| Inset geometry (2 seats) | **Near-minimap ×2**: the one-seat compact block for each seat, mirrored above that seat's radar inside its side-by-side pane, with the same 1:1 rendering; one camera viewscreen, two draw rects (`camera_pane_rects_`) | Maintainer ruling (replaces the centered 1:1 inset at 2 seats; 4 seats keep it) |
...
- Geometry (pinned), **2 seats — near-minimap ×2 (maintainer ruling)**: each seat gets a live-art block from its own UI pane, computed with `compute_view_layout(2, i, seat PREF_VIEW, ui_w, ui_h)`, with exactly that seat's radar width and height and mirrored one radar margin above it. At the classic canvas over scen820, the blocks are `(111, 136, 44, 28)` and `(272, 136, 44, 28)`. Both render the same target 1:1. `screen::camera_pane_rects_` holds the draw geometry; `draw_camera_view_ui` renders each final pane directly. The border and stale-pixel scrub still visit every rect. `camera_docked_`, `layout_pane_count()`, present slices, and the world canvas are unchanged. One `switch` in `relayout_camera_view` owns the style rule: one seat gets one minimap, two seats get two minimaps, and four seats get the centered inset.
- Geometry (pinned), **1 seat — the SECOND MINIMAP (maintainer ruling)**: with one seat the canvas centre is exactly where the seat camera holds the player's own hero, so a centred pane would cover him. The camera therefore mirrors the radar block: same width, height, and x coordinate, stacked one radar margin above it. `radar_block_for_pane(...)` is the single placement anchor. At the classic canvas over scen820, the camera is `(272, 136, 44, 28)` above the radar `(272, 168, 44, 28)`; CENTER COURT is `(271, 142, 45, 25)` above its 45×25 radar. The world is rendered directly into that final raster at denominator 1, so the compact HUD footprint does not shrink the followed ball. No full-scene scratch surface or camera downsample step exists. Docked and four-seat panes use the same 1:1 arithmetic.
