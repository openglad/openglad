...
| D19 | **Gating shape unchanged; no terminal host gate added** | SDL: `is_versus_campaign && picker_lobby_host_controls_visible()` (`picker_team_build.cpp:645-646`). Terminals: `VersusCampaignOnly` only; guests cycle cosmetically and the authoritative echo overwrites (`picker_lobby_network_client.cpp:1635`); the server drops non-host `SettingsChange` (`lobby_server.cpp:1077-1085`). That is the established contract — do not "fix" it here. |
...
One extra pass over `og.oblist()` inside `on_mode_init`, at the point
`own_roster_activation` already walks the oblist: filter
`order() == ORDER_LIVING`, alive, `has_guy()`. AS BUILT, the census
(`record_match_target`) runs at the top of `own_roster_activation` — BEFORE
the troops strip, not after as first drafted. Verified strip-invariant: both
strip paths skip `has_guy()` walkers, so the human census cannot change
across the strip, and `census_mask` guarantees no human roster team falls
outside the Matched mask. **Every `g_*`/stat read is guarded by `has_guy()`** — unguarded
...
| E8 | Onslaught | Sentinel treated as Auto for masks (the `row.teams` manifest default still substitutes); matched power ignored — no plan, no announce, generator armies identical to the Auto twin; the shared census still latches `MATCHED_TARGET` when humans are present (D17, follow-up note filed in the PR). Pinned by `ModesOnslaught.matched_request_masks_like_auto_and_ignores_power`. |
...
- Activation: `census_mask` `mode_match.lua:54-73`; `own_roster_activation`
  `:93-130` (solo case adds the first authored non-roster team, `:120-126`).
