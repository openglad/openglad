...
D41 plan phase): the power census (`census_power`) runs inside
`match.bank_match_target(plan, obs)` at the top of every apply — BEFORE the
troops strip, not after as first drafted — while matched-ness and the
headcount (`MATCHED.SIZE`) are decided in the pure plan
(`match.plan_activation` over the roster head-count inputs; the old
...
- Activation: the D41 plan phase — `match.plan_activation` +
  `match.plan_fills` (`lib/mode_match.lua`), dispatched through each mode's
  `on_mode_plan` and EXECUTED by its `on_mode_init` (the former
  `census_mask`/`own_roster_activation` are deleted; 2026-08-18: the
  solo/roster arms are gone — one backfill rule, request = explicit N or
  the authored count at Auto; see the D26 addendum).
