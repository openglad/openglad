...
---@field on_mode_init? fun(level: integer, plan: og.MatchPlan?)
---@field on_mode_plan? fun(level: integer, inputs: og.MatchPlanInputs): og.MatchPlan?
...
-- on_mode_plan census (build_match_plan_inputs, match_plan.h):
-- request knobs raw (0 = Auto/unset), one row per score team,
-- raw {team, level} flag rows in fx-list order (unfiltered —
-- the first-flag-per-team rule stays in the plan).
---@class og.MatchPlanInputs
---@field team_count integer
---@field strip_troops integer
---@field score_limit integer
---@field respawn_ticks integer
---@field teams og.MatchPlanTeamInputs[]
---@field flags og.MatchPlanFlag[]

---@class og.MatchPlanTeamInputs
---@field anchors integer
---@field roster integer
---@field npcs integer
---@field generators integer

---@class og.MatchPlanFlag
---@field team integer
---@field level integer

-- The plan on_mode_plan answers: consumed verbatim by
-- on_mode_init (apply EXECUTES the plan) and parsed by the
-- engine preview (MatchPlanSummary). fill vocabulary:
-- "company"|"troops"|"bots"|"matched"|"generators"|"empty"
-- (unknown strings render verbatim).
---@class og.MatchPlan
---@field mode string
---@field starts boolean
---@field reason? string
---@field authored_mask integer
---@field active_mask integer
---@field matched boolean
---@field matched_size integer
---@field score_limit integer
---@field seeded_squads boolean
---@field teams og.MatchPlanTeam[]

---@class og.MatchPlanTeam
...
---@class og.CampaignPage
---@field title string
---@field lines? string[]
---@field entries? og.CampaignPageEntry[]

-- picker_action's optional return: `message` is the toast.
...
-- og.campaign_var; the hooks run under the campaign fence
...
---@field campaign_current_level fun(): integer
---@field campaign_gold fun(): integer
...
---@field campaign_state_set fun(key: string, v: integer) # og.campaign_state_set(key, v) — the binding is the bounds-enforcement point: a bad key raises BEFORE the provider runs, and the provider's own check-then-wri...
---@field campaign_team fun(): og.CampaignRosterEntry[] # og.campaign_team() → array of plain tables (values, not handles — the roster outlives any dispatch, so handle semantics would be a trap).
---@field campaign_var fun(name: string): integer # og.campaign_var(name) → the campaign decision value copied into this world at level load (0 when absent — the var==0 rule is what keeps a dedicated server, w...
